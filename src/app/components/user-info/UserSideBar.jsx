...
const UserSideBar = () => {
  ...
  return (
    <div className="h-fit w-[40vw] flex flex-col relative pt-[45px] pb-[150px] px-[58px] rounded-[10px] bg-white border border-[#E6EBF1]">
      <div className="w-full h-fit flex flex-col gap-5 items-center justify-center rounded-[10px]  p-4">
        <img
        ...
        {sideBarButtons.map((button, index) => (
          <Link href={button.url} key={index} className="w-full h-fit">
            <div className="w-full h-fit flex flex-row items-center cursor-pointer justify-between">
              {button.label}
              {pathname === button.url && <MdKeyboardArrowRight />}
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="w-full h-fit flex flex-row text-[12px] font-normal items-center justify-between mt-7">
        <p className="text-[#FF0000]">Системээс гарах</p>
      </div>
